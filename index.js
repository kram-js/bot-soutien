...
client.on("presenceUpdate", (oldPresence, newPresence) => {
    ...
    let guildData = database[newPresence.guild.id] || {};
    if (!guildData.role) return;
    if (!guildData.message) return;
    const activity = newPresence.activities.find((a) => a.type === 4);
    if(!activity && newPresence.member.roles.cache.has(guildData.role)){
        return newPresence.member.roles.remove(guildData.role);
    }
    let words = activity?.state?.toLowerCase().split(" ");
    if(!words) return;
    if(words.some((w) => w.includes(guildData.message.toLowerCase()))){
        newPresence.member.roles.add(guildData.role).catch((e) => {});
    }else if(newPresence.member.roles.cache.has(guildData.role)){
         newPresence.member.roles.remove(guildData.role).catch((e) => {})
    }
})